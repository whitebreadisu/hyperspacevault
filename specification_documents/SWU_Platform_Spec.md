...
Most `/api/*` routes require `Authorization: Bearer <Firebase ID token>`. A single FastAPI dependency, `get_db`, both verifies that token and establishes the caller's tenant context for PostgreSQL Row-Level Security (RLS) — every such router gets both for free by declaring `Depends(get_db)`. The exceptions are the public catalog reads and the BL-205 share-token reads (both below).
...
>
> **As-built addition — BL-205 (share-token viewer reads, App Spec §19.1 authoritative).** A fourth session dependency, **`get_shared_db`**, backs `GET /api/shared/{token}[/quantities|/limits]` — the platform's first unauthenticated read of tenant data. No `Authorization` header is inspected: the ≥128-bit share token is the credential. Resolution runs on the RLS rails — the session starts tenant-less (same GUC clearing as `get_catalog_db`), sets `app.share_token`, and the `shares_token_select` policy (migration `0030`) makes exactly the matching **unrevoked** row visible; only then is `app.current_tenant_id` set to the share's owner tenant, so the two data routes reuse the owner's own service reads unchanged. Invalid and revoked tokens are indistinguishable (404 both); shared reads are per-IP rate-limited (BL-53 cheap tier) and carry `Cache-Control: private, max-age=30`; revocation is soft (`revoked_at`) and immediate; share management (`/api/shares`) is a normal `get_db` tenant surface, and `purge_tenant` clears `shares` on account deletion. Owner-side cardinality: one active share per scope target (partial unique index). *Ships with v1.4 (BL-205).*
>
...
- `_get_firebase_app()` (`app/auth.py:16-24`) — lazily initializes the Firebase Admin SDK via `credentials.ApplicationDefault()`. On Cloud Run this resolves to the `backend-runtime` service account's identity automatically — no Secret Manager entry needed for this step.
- `verify_firebase_token(authorization)` (`app/auth.py:50-63`) — requires `Authorization: Bearer <token>`; calls `auth.verify_id_token(token, app=...)`, which validates signature (against Google's rotating public keys), expiry, issuer, and audience. Raises `HTTPException(401)` on any failure (missing header, wrong scheme, invalid/expired token). Returns `(firebase_uid, email, email_verified)` — the third element (BL-16) is the decoded token's `email_verified` claim, defaulted to `False` if the claim is absent, never treated as an implicit pass.
- `get_current_identity(authorization: Optional[str] = Header(default=None))` (`app/auth.py:66-75`) — the FastAPI dependency itself. Tests override this via `app.dependency_overrides`, so the real Firebase Admin app is never initialized outside a deployment.
- `require_verified_email(identity: tuple[str, str, bool] = Depends(get_current_identity))` (`app/auth.py:102-134`, BL-16, scope widened by BL-54 S1) — a second dependency that composes off `get_current_identity` rather than re-decoding the token, so a route declaring it alongside `Depends(get_db)` still only verifies the token once per request. Raises `HTTPException(403, detail="email_not_verified")` when the third (`email_verified`) element is falsy. Applied to the inventory-mutation routes and, since BL-54, `GET /api/inventory/export` — see the BL-16/BL-54 as-built notes above for the full scope boundary.
...
This is the mechanism that an external review (ChatGPT, 2026-06-14) misread as missing. `app/database.py:147-150`:
...
Confirmed by grep, refreshed 2026-08-16 (BL-231 sweep; earlier snapshots: 2026-07-24 post-BL-102, original 2026-06) — across the current `app/routers/`: **16** occurrences of `Depends(get_db)`: `account.py` (2), `deck_check.py` (1), `inventory.py` (6 — BL-219's `adjust` added one), `settings.py` (2), `shares.py` (5 — the BL-205 owner-side share-management routes); 0 separate auth dependency declared anywhere at the router level. `base_cards.py`/`catalog.py`/`sets.py`/`feedback.py`/`images.py` deliberately run on the tenant-less/optional-auth dependencies (`get_catalog_db`/`get_optional_db`, Section 1.1) instead, and `shares.py`'s anonymous viewer router runs on `get_shared_db` (token-scoped, BL-205) — the mechanism below is unchanged, only which routers exercise it.
...
### 1.4 `get_db` step by step — `app/database.py:147-173`
...
classify-pr ─► [backend / frontend / terraform-fmt run or skip] ─► ci-ok   (BL-178 — see below; ci-ok is PR-only,
             classify-pr runs on push too as a ~3s constant so the implicit success() needs-chain stays intact)
```
...
**Reworked 2026-07-15 (BL-131).** Through 2026-07-14 the human gate was a `promote-prod` job at the end of the main pipeline, held by the GitHub "production" environment's required-reviewers rule. The repo going private (2026-07-14) silently removed that rule — environment protection is Enterprise-only on private repos (verified empirically: the API re-add returns 422 on Pro). Promotion was reworked rather than paying Enterprise for one rule. **Update 2026-07-29 (BL-172):** the repo is public again and environment protection is available once more — the dispatch mechanism below is retained by owner decision: it already implements the intended policy (explicit owner-authorized promotes plus the `risk:low` fast lane) with no platform-tier dependency. **Update 2026-08-13 (BL-210):** the dispatch now runs a **release-notes gate** before terraform — at the promoted SHA, the newest `releaseNotes.ts` entry must match `frontend/package.json`'s version (trailing-`.0` normalized) and carry promote-day's America/Chicago date, else the promote fails before touching prod (`allow_stale_notes` dispatch input waives only the date check, for rollbacks/re-promotes); after a successful promote, the workflow mirrors the shipped entry to a **GitHub Release** on the `v{version}` tag (creating the tag at the promoted SHA if absent; mirror failure never fails the promote). Tooling: `.github/scripts/release_notes.mjs`.
...
- `google_iam_workload_identity_pool_provider.github` — provider id `github`, OIDC issuer `https://token.actions.githubusercontent.com`, attribute mapping `google.subject = assertion.sub`, `attribute.repository = assertion.repository`, `attribute_condition = "assertion.repository == 'whitebreadisu/hyperspacevault'"`.
- `google_service_account_iam_member.terraform_ci_wif` — grants `roles/iam.workloadIdentityUser` on `terraform-ci` to `principalSet://iam.googleapis.com/projects/<project_number>/locations/global/workloadIdentityPools/github-actions/attribute.repository/whitebreadisu/hyperspacevault`.
...
**D4 — prod Cloud SQL tier (BL-150 owner decision, 2026-07-24 triage, `Definition_Steadying_2026-07-24.md` §2; merged 2026-07-25, PR #445).** A5-05 found prod's `db-f1-micro` memory pinned at a constant 100% utilization (including OS page cache) with only 25 `max_connections`. Decision: prod moves to **`db-g1-small`** (~3× memory, `max_connections` 25→50, ~+$18/mo) — set explicitly on prod's module call in `terraform/environments/prod/main.tf` rather than changing the module default, since dev deliberately stays on `db-f1-micro` (a real per-env difference, not a stale default). **State: APPLIED — prod runs `db-g1-small`** (rode a subsequent owner-gated prod promote; live-verified 2026-08-01 during the BL-21 restore drill, where the tier showed up directly in the numbers: the prod PITR clone reached RUNNABLE 2.6× faster than dev's f1-micro rehearsal despite more data). The apply was in-place (no instance recreation) with a brief instance restart. **Cross-reference:** the SQLAlchemy connection-pool caps merged the same night (admin engine 1+1, app engine 3+3 → worst case 8 connections/instance, 24 at prod's `max_instance_count = 3`) were sized to stay safe under `db-f1-micro`'s 25-connection ceiling *and* trivially safe once D4's `db-g1-small` move lands (50-connection ceiling) — see `backend/app/database.py` and BL-150 W0 (PR #425).

**Recovery objectives (RR-7).** Targets for user-data loss and downtime, set 2026-07-08: **RPO ≤ 1 hour** (PITR's WAL archiving makes the realistic recovery point minutes, but 1h is the commitment) and **RTO ≤ 4 hours** (manual restore: clone prod from a PITR timestamp via `gcloud sql instances clone --point-in-time`, verify row counts, repoint the app). These are honest solo-operator numbers — the constraint is operator availability, not tooling. **Both restore drills have been performed (BL-21 closed):** dev rehearsal 2026-07-30 (`swu-dev-pg`, restore-to-queryable ~26 min on db-f1-micro) and the prod drill 2026-08-01, owner present (`swu-prod-pg`, **restore-to-queryable ~9m 14s** — PITR clone accepted → RUNNABLE in 8m 39s, connectable ~35s later; 17/17 tables verified identical to a baseline read at the PITR point, incl. 3.49M `variant_prices` rows; clone deleted, source untouched). Dev's longer time was tier-dominated, not data-size-dominated. RTO headroom ~26× against the 4h target. Measured commands, timings, and gotchas live in the drill runbook (RR-8, internal repo: `analysis/DB_Restore_Drill_Runbook_2026-07-30.md`).
...
| `postgres-role-password` | Cloud SQL built-in `postgres` role password (rotated during BL-135's owner-access setup, 2026-07-21) — **operator-only, entirely out-of-band** (no Terraform resource at all, unlike the PAT's placeholder-shell pattern); exists in both envs | none — owner ADC ops only |

**Authoritative secrets for manual ops (BL-176, 2026-07-30): the URL secrets.** `database-url` / `app-database-url` are what everything running actually consumes, and their embedded passwords are the live credentials by definition. The raw `*-password` secrets are Terraform bootstrap artifacts that must *match* them — and for 9 days did not: BL-135's 2026-07-21 session rotated the **`postgres` role's** password and stored it into `db-password` (both envs), overloading the `swu_user`-designated secret with a different role's credential. Discovered 2026-07-28 (a prod peek's direct auth failed), reconciled 2026-07-30: the postgres credential moved to its own `postgres-role-password` secret, `db-password` got a new version matching `swu_user`'s live password (verified equal to the DSN's, both envs), and the mislabeled version was disabled (not destroyed — reversible). Terraform state was never wrong — the stray version was an out-of-band add, so no plan diff ever surfaced it. Rule going forward: a rotated credential gets stored in a secret **named for its role**, never in whichever secret is handy.
...
- `data.google_firebase_web_app_config.default` — reads the Web App's `apiKey`/`authDomain`, exposed as Terraform outputs `firebase_web_app_api_key` / `firebase_web_app_auth_domain`, consumed by `ci.yml`'s `frontend-deploy` job (2.6). **BL-211 (2026-08-13):** the `auth_domain` half is overridden per env (`var.auth_domain_override`) to the environment's own Hosting domain — prod `www.hyperspacevault.com`, dev `swu-dev-jbapps.web.app` — so the `/__/auth/*` handler is same-origin with the served app; the default `<project>.firebaseapp.com` value made the handler iframe third-party, which Safari's ITP storage partitioning breaks per-device over time (Google sign-in failures with no correlated deploy). Requires each env's `https://<domain>/__/auth/handler` on the OAuth web client's authorized redirect URIs (console-only edit, same terraform-vs-console drift class as the Google-provider note below).
...
> **Review currency (2026-08-16, BL-232):** a follow-up point-in-time review of the surfaces added after the P7 review — BL-205 share links (the first unauthenticated tenant-data reads), the release-notes system, the public repository, and the v1.4 API additions — was performed 2026-08-16 (working record in the private engineering archive, findings tracked via issue #153). Verdict: no exploitable vulnerability; the resulting hardening batch (share-token log redaction, `/shared/**` robots exclusion, SPA referrer policy, explicit CI workflow permissions, `react/no-danger` lint pin) shipped in PR #155.
